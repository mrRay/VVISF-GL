...
class ISFScene : public GLScene	{
	...
	public:
		...
		//virtual GLBufferRef createAndRenderABuffer(const VVGL::Size & inSize=VVGL::Size(640.,480.), const GLBufferPoolRef & inPoolRef=GetGlobalBufferPool());
		virtual GLBufferRef createAndRenderABuffer(const VVGL::Size & inSize, map<int32_t,GLBufferRef> * outPassDict=nullptr, const GLBufferPoolRef & inPoolRef=GetGlobalBufferPool());
	...
	private:
		using GLScene::createAndRenderABuffer;	//	ISFScene::createAndRenderABuffer overloads this virtual function in GLScene, which hides it- declaring that we use it as 'private' silences the warning from the compiler while also ensuring that clients of ISFScene cannot call GLScene::createAndRenderABuffer
};
