...
void main()	{
	...
	bool		isThisFragInResizedInputImage = (normMaskSrcCoord.x>=0.0 && normMaskSrcCoord.x<=1.0 && normMaskSrcCoord.y>=0.0 && normMaskSrcCoord.y<=1.0) ? true : false;
	...
	vec4		inputImagePixel = (isThisFragInResizedInputImage) ? IMG_PIXEL(inputImage, pixelMaskSrcCoord) : vec4(0,0,0,0);
	...
	float		TTO = (viewAlpha==true || !isThisFragInResizedInputImage) ? inputImagePixel.a : 1.0;
