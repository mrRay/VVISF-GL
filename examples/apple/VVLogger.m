...
- (void) _cleanUpExistingLogs	{
	...
	if ([sortedLogFiles count]>maxNumLogs)	{
		unsigned long			deleteCount = [sortedLogFiles count] - maxNumLogs;
...
- (NSString *) logDir	{
	...
	return [RealHomeDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"Library/Logs/%@",logFolderName]];
}
