...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification	{
	...
	[outputView useFile:[[NSBundle mainBundle] pathForResource:@"AlphaOverCheckerboard" ofType:@"fs"]];
	...
	*/
	...
	NSOpenGLPixelFormat		*format = (sharedContext==nullptr) ? nil : [[[NSOpenGLPixelFormat alloc] initWithCGLPixelFormatObj:sharedContext->pxlFmt] autorelease];
...
- (void) _loadFilterList	{
	...
	if (tmpPath!=nil && [fm fileExistsAtPath:tmpPath isDirectory:&isDirectory] && isDirectory)	{
		...
		auto			dirContents = CreateArrayOfISFsForPath(string([tmpPath UTF8String]), ISFFileType_None, false);
		if (dirContents != nullptr)	{
			for (const auto & tmpPath : *dirContents)	{
				NSString		*tmpString = [NSString stringWithUTF8String:tmpPath.c_str()];
				if (tmpString != nil)
					[filterList addObject:tmpString];
			}
		}
...
- (IBAction) outputShowAlphaToggleUsed:(id)sender	{
	@synchronized (self)	{
		...
		*/
		ISFSceneRef		scene = [outputView scene];
		if (scene != nullptr)	{
			scene->setValueForInputNamed(ISFBoolVal(([sender intValue]==NSOnState)?true:false), string("viewAlpha"));
		}
	}
}
/*
...
- (IBAction) videoSourcePUBUsed:(id)sender	{
	...
	@synchronized (self)	{
		if (repObj != nil)	{
			...
			//}
			if ([repObj isKindOfClass:[NSString class]])	{
