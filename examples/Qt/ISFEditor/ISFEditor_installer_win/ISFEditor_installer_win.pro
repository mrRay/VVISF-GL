...
win32	{
	...
	CONFIG(release, debug|release)	{
		...
		offline_installer_creator.commands += $$BINARYCREATOR -c $$shell_quote($$shell_path($$PWD/config/config.xml)) -p $$shell_quote($$shell_path($$OUT_PWD/packages)) ${QMAKE_FILE_OUT} $$escape_expand(\n)
		#offline_installer_creator.commands += $$BINARYCREATOR --offline-only -c $$shell_quote($$shell_path($$PWD/config/config.xml)) -p $$shell_quote($$shell_path($$OUT_PWD/packages)) ${QMAKE_FILE_OUT} $$escape_expand(\n)
		...
		repo_creator.commands += if exist $$shell_quote($$shell_path($$OUT_PWD/repository)) RMDIR /S /Q $$shell_quote($$shell_path($$OUT_PWD/repository)) $$escape_expand(\n)
		#	create a new repository directory
		repo_creator.commands += $$QMAKE_MKDIR $$shell_quote($$shell_path($$OUT_PWD/repository)) $$escape_expand(\n)
		#	populate the repository directory with the repogen tool
		repo_creator.commands += $$REPOGEN -p $$shell_quote($$shell_path($$OUT_PWD/packages)) $$shell_quote($$shell_path($$OUT_PWD/repository)) $$escape_expand(\n)
