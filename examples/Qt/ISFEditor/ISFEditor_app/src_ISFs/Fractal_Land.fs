/*
{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/XsBXWt by Kali.  :D",
    "IMPORTED": {
        "iChannel1": {
            "NAME": "iChannel1",
...
vec4 nyan(vec2 p)
{
	...
	vec4 color = IMG_NORM_PIXEL(iChannel1,mod(vec2(uv.x/3.0+210.0/256.0-nt+0.05,.5-uv.y-ny),1.0));
...

{
	...
	dir.y-=.02;
	float sunsize=7.-max(0.,IMG_NORM_PIXEL(iChannel0,mod(vec2(.6,.2),1.0)).x)*5.; // responsive sun size
...
void main() {
	if (PASSINDEX == 0)	{
		...
		vec2 mouse=(iMouse.xy/RENDERSIZE.xy-.5)*3.;
		mouse=vec2(0.,-0.05);
