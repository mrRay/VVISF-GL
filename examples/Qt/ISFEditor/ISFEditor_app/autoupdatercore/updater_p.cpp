...
bool UpdaterPrivate::startUpdateCheck()
{
	...
	if(running)
		...
		/*
		connect(mainProcess, &QProcess::readyReadStandardOutput, this, [&]()	{
			qDebug() << "QProcess::readyReadStandardOutput()";
			qDebug() << "\t...about to read stdout to a string...";
			QString		stdOutStr = QString(mainProcess->readAllStandardOutput());
			qDebug() << "\tstdOutStr is " << stdOutStr;
		});
		connect(mainProcess, &QProcess::readyReadStandardError, this, []()	{
			qDebug() << "QProcess::readyReadStandardError()";
			
		});
		connect(mainProcess, &QProcess::stateChanged, this, [](QProcess::ProcessState newState){
			qDebug() << "QProcess:stateChanged(), new state is " << newState;
		});
		*/
		
		//	make a TTL timer
		mainProcessTTL = new QTimer(mainProcess);
		mainProcessTTL->setInterval(10000);
		
		
		//	kill the update process when the TTL timer exits
		connect(mainProcessTTL, &QTimer::timeout, [&]()	{
			qDebug() << "mainProcessTTL executing";
			if (mainProcess != nullptr && mainProcess->state() != QProcess::NotRunning)	{
				mainProcess->kill();
...
void UpdaterPrivate::updaterReady(int exitCode, QProcess::ExitStatus exitStatus)
{
	...
	if(mainProcess) {
		...
		if (mainProcessTTL != nullptr)	{
			disconnect(mainProcessTTL, &QTimer::timeout, 0, 0);
			if (mainProcessTTL != nullptr)	{
				mainProcessTTL->stop();
			}
		}
