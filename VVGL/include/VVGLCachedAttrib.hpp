...
struct VVGLCachedAttrib	{
	...
	public:
		//VVGLCachedAttrib();
		VVGLCachedAttrib(string & inName);
		VVGLCachedAttrib(const string & inName);
		VVGLCachedAttrib(const VVGLCachedAttrib & n) : loc(n.loc), name(n.name), prog(n.prog) {}
	public:
	...
	public:
		...
		void disable();
		inline string getDescriptionString() const { return FmtString("<VVGLCachedAttrib \"%s\", %d>",this->name.c_str(),this->loc); }
		friend ostream & operator<<(ostream & os, const VVGLCachedAttrib & n) { os<<n.getDescriptionString();return os; }
};
