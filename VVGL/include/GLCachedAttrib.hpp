#ifndef VVGL_GLAttrib_h
#define VVGL_GLAttrib_h
...
/*	the goal of this class is to create an object that fetches & caches the location of an attribute in 
...
want to track the location of an attrib.				*/
...
struct GLCachedAttrib	{
	...
	public:
		//GLCachedAttrib();
		GLCachedAttrib(string & inName);
		GLCachedAttrib(const string & inName);
		GLCachedAttrib(const GLCachedAttrib & n) : loc(n.loc), name(n.name), prog(n.prog) {}
	public:
	...
	public:
		...
		void disable();
		inline string getDescriptionString() const { return FmtString("<GLCachedAttrib \"%s\", %d>",this->name.c_str(),this->loc); }
		friend ostream & operator<<(ostream & os, const GLCachedAttrib & n) { os<<n.getDescriptionString();return os; }
};
